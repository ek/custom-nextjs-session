...
export const otpInsertSchema = createInsertSchema(otpCodeTable, {
  ...
  hashedCode: z.string().length(64, { message: "Hashed OTP code must be exactly 64 characters" }),
  createdAt: z.date().optional()
});
export const otpUpdateSchema = createUpdateSchema(otpCodeTable, {
  hashedCode: z.string().length(64, { message: "Hashed OTP code must be exactly 64 characters" }).optional(),
});
